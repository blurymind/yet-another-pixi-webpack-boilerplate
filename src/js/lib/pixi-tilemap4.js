...
(function (pixi_tilemap) {
    var CompositeRectTileLayer = (function (_super) {
        ...
        CompositeRectTileLayer.prototype.addRect = function (textureIndex, u, v, x, y, tileWidth, tileHeight, animX, animY, rotate, framesX) {
            ...
            if (this.children[childIndex] && this.children[childIndex].textures) {
                this.children[childIndex].addRect(textureId, u, v, x, y, tileWidth, tileHeight, animX, animY, rotate, framesX);
            }
        };
        CompositeRectTileLayer.prototype.addFrame = function (texture_, x, y, animX, animY, framesX, framesY) {
            console.log(animX, framesX)
            // if(animX && framesX) animX = animX  % framesX
            var texture;
            var layer = null;
            var ind = 0;
            ...
            layer.addRect(ind, texture.frame.x, texture.frame.y, x, y, texture.orig.width, texture.orig.height, animX, animY, texture.rotate, framesX);
            return true;
        };
...
(function (pixi_tilemap) {
    ...
    pixi_tilemap.POINT_STRUCT_SIZE = 11;//changed
    var RectTileLayer = (function (_super) {
        ...
        RectTileLayer.prototype.addRect = function (textureIndex, u, v, x, y, tileWidth, tileHeight, animX, animY, rotate, framesX) {
            if (animX === void 0) { animX = 0; }
            if (animY === void 0) { animY = 0; }
            if (rotate === void 0) { rotate = 0; }
            if (framesX === void 0) { framesX = 0; }
            var pb = this.pointsBuf;
            this.hasAnim = this.hasAnim || animX > 0 || animY > 0;
            console.log("pushed", framesX)
            pb.push(u);
            pb.push(v);
            pb.push(x);
            pb.push(y);
        ...
        RectTileLayer.prototype.renderWebGLCore = function (renderer, plugin) {
            ...
            if (this.modificationMarker !== vertices) {
                ...
                for (var i = 0; i < points.length; i += pixi_tilemap.POINT_STRUCT_SIZE) {
                    ...
                    var framesX = points[i + 10];
                    console.log("anim", animX, "frames", framesX)
                    // if (animX && framesX) {
                    //     animX = animX % framesX;
                    // }
...
(function (pixi_tilemap) {
    ...
    var RectTileShader = (function (_super) {
        ...
        function RectTileShader(gl, maxTextures) {
            ...
            _this.vertSize = pixi_tilemap.POINT_STRUCT_SIZE + 1 //changed;
            _this.vertPerQuad = 4;
        ...
        RectTileShader.prototype.createVao = function (renderer, vb) {
            var gl = renderer.gl;
            console.log("VB",vb,this.indexBuffer, this.attributes, this.stride, gl)
            return renderer.createVao()
                ...
                .addAttribute(vb, this.attributes.aFramesX, gl.FLOAT, false, this.stride, 11 * 4);
        };
...
(function (pixi_tilemap) {
    ...
    (function (shaderGenerator) {
        ...
        function generateSampleSrc(maxTextures) {
            console.log("shader",maxTextures)
            var src = '';
            src += '\n';
            src += '\n';
